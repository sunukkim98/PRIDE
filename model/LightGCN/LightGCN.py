...
class LightGCN(nn.Module):
    def __init__(self, config, dataset):
        ...
        self.norm_adj = dataset.gcn_graph().to(self.config["device"])
    ...
    def forward_vq(self, user_list, pos_items, neg_items):
        ...
        return user_r[torch.LongTensor(user_list).to(self.config["device"]), :], \
               item_r[torch.LongTensor(pos_items).to(self.config["device"]), :], \
               item_r[torch.LongTensor(neg_items).to(self.config["device"]), :], reg, item_r 
    ...
    def get_user_emb(self, user_list):
        user_r, _ = self._get_rep()
        return user_r[torch.LongTensor(user_list).to(self.config["device"]), :]

    def get_item_emb(self, item_list):
        _, item_r = self._get_rep()
        return item_r[torch.LongTensor(item_list).to(self.config["device"]), :]
