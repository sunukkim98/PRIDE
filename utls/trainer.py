...
class REQUIEMCFTrainer(CFTrainer):
    """
    REQUIEM trainer for implicit-feedback CF.
    After `begin_adv`, reweights each (u, i+) sample by:
      - user intent: cosine(user centroid from codebook-attn, positive item embedding)
      - item/cluster stability: inverse movement of codebook centroids
    Final weight = stability * intent (min-max normalized).
    """
    def __init__(self, trainer_config) -> None:
        super().__init__(trainer_config)
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_data in self.dataloader:
            ...
            raw_loss = F.softplus(neg_logits - pos_logits).detach()
            ...
            if epoch >= self.begin_adv:
                ...
                epoch_log["epoch"].extend([epoch] * len(user_id_list))
                epoch_log["user"].extend(np.array(user_id_list).tolist())
                epoch_log["pos_item"].extend(np.array(pos_item_list).tolist())
                epoch_log["raw_loss"].extend(raw_loss.cpu().numpy().tolist())
                epoch_log["cluster_stability"].extend(cluster_stability.detach().cpu().numpy().tolist())
                epoch_log["cosine_sim"].extend(cosine_sim.detach().cpu().numpy().tolist())
                epoch_log["weight"].extend(weights.detach().cpu().numpy().tolist())
                epoch_log["is_noisy"].extend(np.array(is_noisy_list).tolist())
            else:
        ...
        if epoch >= self.begin_adv:
            ...
            log_path = os.path.join(
                log_dir,
                f"noise_{self.config['noise']}_seed_{self.config['seed']}_epoch_{epoch}.csv"
            )
...
class HybridREQUIEMCFTrainer(CFTrainer):
    """
    Hybrid REQUIEM trainer for implicit-feedback CF.
    At 'begin_adv', reweights each (u, i+) sample by:
      - R-CE
    After `begin_adv`, reweights each (u, i+) sample by:
      - user intent: cosine(user centroid from codebook-attn, positive item embedding)
      - item/cluster stability: inverse movement of codebook centroids
    ...
    def __init__(self, trainer_config) -> None:
        ...
        # self.log_dir = f"weights_hist/{self.config['model']}/{self.config['method']}/{self.config['dataset']}"
        # os.makedirs(self.log_dir, exist_ok=True)

    def _pick_hist_epochs(self):
        # warm-up epochs: 0 ~ begin_adv-1
        warm_start = 0
        warm_end = max(0, self.begin_adv - 1)
        warm_mid = (warm_start + warm_end) // 2

        # requiem epochs: begin_adv ~ n_epochs
        rq_start = self.begin_adv
        rq_end = self.n_epochs - 1
        rq_mid = (rq_start + rq_end) // 2

        return {
            "warm": {warm_start, warm_mid, warm_end},
            "rq": {rq_start, rq_mid, rq_end},
        }

    def _save_hist_png(self, values_cpu: torch.Tensor, title: str, outpath: str, bins: int = 80):
        arr = values_cpu.numpy()
        plt.figure()
    ...
    def _train_epoch(self, epoch):
        ...
        # # histogram plot epochs
        # picked = self._pick_hist_epochs()
        # is_warm_epoch = (epoch < self.begin_adv) and (epoch in picked["warm"])
        # is_rq_epoch = (epoch >= self.begin_adv) and (epoch in picked["rq"])
        # rce_bank = []
        # rq_bank = []
        ...
        for batch_data in self.dataloader:
            ...
            if self.config["model"] in ["NeuMF"]:
                pos_logits, neg_logits, users_emb, pos_items_emb, neg_items_emb, l2_norm_sq = self.model(user_id_list, pos_item_list, neg_item_list)
                pos_logits = torch.sum(pos_logits, dim=1)
                neg_logits = torch.sum(neg_logits, dim=1)
            else:
                users_emb, pos_items_emb, neg_items_emb, l2_norm_sq, all_items = self.model.forward_vq(user_id_list, pos_item_list, neg_item_list)
            ...
            if epoch >= self.begin_adv:
                ...
                # # collect only for selected rq epochs
                # if is_rq_epoch:
                #     rq_bank.append(weights.detach().float().cpu())
            else:
                weights = self.get_rce_weight(pos_logits, neg_logits).to(self.device).detach()

                # # collect only for selected warm-up epochs
                # if is_warm_epoch:
                #     rce_bank.append(weights.detach().float().cpu())
        ...
        # if is_warm_epoch and len(rce_bank) > 0:
        #     vals = torch.cat(rce_bank, dim=0)
        #     out = os.path.join(self.log_dir, f"warm_rce_weight_epoch{epoch:04d}.png")
        #     self._save_hist_png(vals, f"Warm-up R-CE weight | epoch={epoch}", out)

        # if is_rq_epoch and len(rq_bank) > 0:
        #     vals = torch.cat(rq_bank, dim=0)
        #     out = os.path.join(self.log_dir, f"requiem_rq_weight_epoch{epoch:04d}.png")
        #     self._save_hist_png(vals, f"REQUIEM RQ weight | epoch={epoch}", out)
        
        end_t = time.time()
        print(f"Epoch {epoch}: Rec Loss: {epoch_loss/len(self.dataloader):.4f}, Time: {end_t-start_t:.2f}")  

class WeightedIntentRQCFTrainer(CFTrainer):
    def __init__(self, trainer_config) -> None:
        super().__init__(trainer_config)
        self.num_codebook = self.config["model_config"]['denoise_config']['num_codebook']
        self.device = self.config["device"]
...
class ItemStableCFTrainer(CFTrainer):
    def __init__(self, trainer_config) -> None:
        super().__init__(trainer_config)
        self.num_codebook = self.config["model_config"]['denoise_config']['num_codebook']
        self.device = self.config["device"]
